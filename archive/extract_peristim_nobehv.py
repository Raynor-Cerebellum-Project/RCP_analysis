...
def extract_one_file(aligned_path: Path) -> None:
    ...
    if stim_ms is None and "ir_ms" in aligned_npz.files:
        stim_ms = np.asarray(aligned_npz["ir_ms"], float).ravel()
        if stim_ms.size:
    ...
    if stim_ms is None:
        print(f"[extract] {aligned_path.name}: no stim_ms or ir_ms; skipping.")
        return
