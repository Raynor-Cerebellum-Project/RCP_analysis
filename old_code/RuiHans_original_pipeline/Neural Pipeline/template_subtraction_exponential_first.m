function amplifier_data_cleaned = template_subtraction_exponential_first(amplifier_data, trigs, chan, params, template_mode, plot_chan)
...
for r = 1:num_repeats
    ...
    if range_start > 0 && range_end <= length(amplifier_data)
        ...
        fit_y = smooth_block;
        % fit_y = gaussfilt(fit_x, fit_y_temp, 1);
        ...
        obj_fun = @(p) sum(abs(fit_fun(p, fit_x) - fit_y));

        % Call fmincon
        try
            p_fit = fmincon(obj_fun, p0, [], [], [], [], lb, ub, [], opts);
            ...
            amplifier_data_drift_removed(full_range_start:range_end) = ...
                amplifier_data_drift_removed(full_range_start:range_end) - decay_fit;
            ...
            if r == 1 && chan == plot_chan
                ...
                fit_fun_1 = fit_fun;
                amplifier_data_pre_decay = raw_block;
            end
        catch
            warning('Fit failed for block %d (fmincon)', r);
        end
...
for i = 1:NSTIM
    ...
    chn_data(i, :) = amplifier_data_drift_removed(segment);
end
...
if exist('p_fit_1', 'var') && chan == plot_chan
    ...
    raw_block_original_1 = double(amplifier_data(full_range_start_1:range_end_1));
    full_decay_fit = zeros(size(raw_block_original_1));
    decay_core = fit_fun_1(p_fit_1, fit_x_1);
    decay_core = decay_core - mean(decay_core(end-4:end));  % subtract tail bias
    decay_start_idx = range_start_1 - full_range_start_1 + 1;
    full_decay_fit(decay_start_idx:decay_start_idx+length(decay_core)-1) = decay_core;
    ...
    plot(t_ms, raw_block_original_1, 'Color', [0.8 0.3 0.3 0.4], ...
        'DisplayName', 'Original Raw');
    plot(t_fit_ms, fit_y_1, 'Color', [0.1 0.6 0.1 0.7], ...
        'LineWidth', 1.2, 'DisplayName', 'Low-pass Fit Input');
    plot(t_ms, full_decay_fit, 'k-', 'DisplayName', 'Full Decay Fit');
