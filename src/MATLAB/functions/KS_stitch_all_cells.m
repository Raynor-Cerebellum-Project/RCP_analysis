...
load([filepath '\Sorting results\KiloSort\rez.mat'])
% rez = get_rez_xy(rez,[filepath '\Sorting results\KiloSort\pc_features.npy']); %<-uncomment if using kilo2.5 output, also change to appropriate directories
...
ST = double(readNPY([filepath '\Sorting results\KiloSort\spike_times.npy']));
[~,I1] = sort(ST1);
[~,I2] = sort(ST);
[~,I3] = sort(I2);
I = I1(I3);
CH = CH(I);
SC = double(readNPY([filepath '\Sorting results\KiloSort\spike_clusters.npy']));
SC = SC+1;
cluster_numbers = unique(SC);
% [data, ~, raw] = tsvread([Path_name '..\KiloSort\cluster_info.tsv'] );
[data, ~, raw] = tsvread([filepath '\Sorting results\KiloSort\cluster_info.tsv'] );
% CH = zeros(size(SC));
...
for file_index = 1:length(file_names)
    ...
    for cell_index = 1:length(cluster_numbers)
        ...
        cluster_name = cluster_numbers(cell_index); 
        Data = Data_back;
...
%         newname = 'ua';   
        newname = ['ua_' num2str(cluster_name)]; %USES NAME OF NEURON
...
%         newname = 'fr';
        newname = ['fr_' num2str(cluster_name)]; %USES NAME OF NEURON
        Data.(newname) = fr;
        Data.ChannelList(end+1)={newname};
        try
            Data.ChannelNames(end+1,:)='                ';
...
%         warning off  %don't make new file/directory
%         mkdir([Path_name '\..\Seperate cells\Kilosort_all_cells\' trackname '_CELL_' num2str(mainclusterSite) '_kilo_'  num2str(cluster_number-1)  '_' quality{cell_index}])
%         warning on
        
        
        
    end
    mkdir([Path_name '\..\Seperate cells\Kilosort_all_cells\' trackname '_CELL_' num2str(mainclusterSite) '_kilo_'  num2str(cluster_number-1)  '_noquality'])
    %need to fix to not re-make that directory every time, and change to
    %mor intuitive directory
    save([Path_name '\..\Seperate cells\Kilosort_all_cells\' trackname '_CELL_' num2str(mainclusterSite) '_kilo_'  num2str(cluster_number-1)  '_noquality', '\' file_name],'Data','-v7.3');
