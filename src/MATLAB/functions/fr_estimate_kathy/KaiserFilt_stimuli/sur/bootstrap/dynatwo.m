...
in1 = uicontrol('Parent',h0, ...
    ...
    'Callback','k = []; sig = []; k = get(in1, ''value''); if (k == 1), in1to4(:,1) = 0; model(1,:) = ''---''; else, lat_t = lat; lat = 0; sig = eval(Signaldefinitions(listfind((Plotlist(k-1,:)), Signals),:)); if ( (Plotlist(k-1,3) == ''v'') | (Plotlist(k-1,1:3) == ''vcj'') | (Plotlist(k-1,1:3) == ''vvg'') ); in1to4(2:length(sig)+1,1) = sig; else; in1to4(1:length(sig),1) = sig; end; lat = lat_t; model(1,:) = (Plotlist(k-1,1:3)); end; set(in2d, ''Value'', 0); set(in2a, ''Value'', 0);set(ig1, ''Value'', 0);', ...
    'Units','points', ...
    'ListboxTop',0, ...
...
in3 = uicontrol('Parent',h0, ...
    'String',['none|' makelist1(Plotlist,NoofDisplayed)], ...
    'Callback','l = []; sig = []; l = get(in3, ''value''); if (l == 1), in1to4(:,3) = 0; model(2,:) = ''---''; else, lat_t = lat; lat = 0; sig = eval(Signaldefinitions(listfind((Plotlist(l-1,:)), Signals),:)); if ( (Plotlist(l-1,3) == ''v'') | (Plotlist(l-1,1:3) == ''vcj'') | (Plotlist(l-1,1:3) == ''vvg'') ); in1to4(2:length(sig)+1,3) = sig; else; in1to4(1:length(sig),3) = sig; end; lat = lat_t; model(3,:) = (Plotlist(l-1,1:3)); end;set(in4d, ''Value'', 0); set(in4a, ''Value'', 0);set(ig3, ''Value'', 0);', ...
    'Units','points', ...
    'ListboxTop',0, ...
...
outs = uicontrol('Parent',h0, ...
    'String',['none|' makelist1(Plotlist,NoofDisplayed)], ...
    'Callback','m = []; sig = []; m = get(outs, ''value''); if (m == 1), out1(:,1) = 0; model(5,:) = ''---''; else, lat_t = lat; lat = 0; sig = eval(Signaldefinitions(listfind((Plotlist(m-1,:)), Signals),:)); out1(1:length(sig),1) = sig; lat = lat_t; model(5,:) = (Plotlist(m-1,1:3)); if ((Plotlist(m-1,1:3) ~= '' fr'')); mode_flag = 1234; end; end;set(ig5, ''Value'', 0);', ...
    'Units','points', ...
    'ListboxTop',0, ...
...
h1 = uicontrol('Parent',h0, ...
    'String','Get Latency', ...
    'Callback','clear ho h1 in1 in2 in3 in4 outs sig; close; [x,se,error_sim,VAF,BIC,est,Others] = dynlatpick(M,ns,lat,out1,in1to4,mode_flag); M=M+lat-Others.lat;lat=Others.lat; plotdyn; clear filename_e out1 in1to4 model; ', ...
    'Fontsize',9, ...
    'Units','points', ...
    'ListboxTop',0, ...
