...
for i=1:length(ns);
    startpt = -10;		%set range of latencies to check for
    endpt = 0;
...
%    keyboard
    ...
    shiftedehv=[shiftedehv;newehv(oldM(ns(i),1)-values(i,2):oldM(ns(i),2)-values(i,2))];
    newshiftedehv=[newshiftedehv;newehv(oldM(ns(i),1)-values(i,2):oldM(ns(i),2)-values(i,2))-values(i,4)];
    plot_hhv=[plot_hhv;newhhv(oldM(ns(i),1):oldM(ns(i),2))];
