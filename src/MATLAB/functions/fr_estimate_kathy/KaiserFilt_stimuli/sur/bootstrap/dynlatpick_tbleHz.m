...
for i=1:length(ns);
    startpt = -20;		%set range of latencies to check for
    endpt = 20;
    ...
    for w = 1:sizelistlat
        ...
        %        gain_h=[gain_h; x];
        results(w,1) = ns(i);     %write the ns in the 1st column of results matrix
        results(w,5) = VAF;		%write the VAF in the 5th column
        results(w,4) = x(1);    %write the bias in the 4th column
        results(w,3) = x(2);    %write the hhv gain in the 2nd column
    end;
    warning on
...
disp('----------------------------------------------------------------------------------------');
disp(sprintf('mean\t %0.4f\t %0.4f', mean_latency, mean_hhv_gain)); 
disp(sprintf('SE\t %0.4f\t %0.4f', SE_latency, SE_hhv_gain)); 
