function [x,se,error_sim,VAF,BIC,est,Others] = dodyn2(M,ns,lat,out1,in1to4,mode_flag)
...
error_sim = mean(errmdyn2(x,t_out,t_in));
NN=length(t_out);
...
% added by SSG for getting the phase difference between ehv & hhv during buzzes

phase = atan(X(3)/X(2))*(180/pi)
