function [t1,p1,w1,w2,vl,v,f,V1,st]=soebw(z,ix,t,g,nn,ns,FT,lim,Vcap,M)
...
%  Define limit array
   ...
   l=0;k=1;V1=Vcap+1; nt=nf+nb+1; st=0;
...
% loop up to 10 times:
  ...
  while [V1 > Vcap l<10],
     ...
     f=fstab([1 t1(nb+1:nt-1)']);t1(nb+1:nt-1)=f(2:nf+1)';

%    Compute estimate of the output:

     if FT>2,

	y = z(1:Length,1) - t1(nt)*ones(Length,1);
        u = z(1:Length,2);
        [v,w1,w2,p1] = peoew( b, nb+nk-1, f, nf, [y,u], M, FT);

     else

	y = z(1:Length,1) - t1(nt)*ones(Length,1);
        u = z(1:Length,2);
        [v,w1,w2] = peoew( b, nb+nk-1, f, nf, [y,u], M, FT, p1);
