...
[base_root, code_root, base_folder] = set_paths_cullen_lab(session);  % e.g., '/Volumes/cullenlab_server'
...
if isempty(gcp('nocreate'))
    parpool('IdleTimeout', 120);  % keep pool alive 120 minutes
end
...
for i = 1:length(valid_trials)
    ...
    stim_path = fullfile(trial_path, 'stim_data.mat');
    stim_struct = load(stim_path);
    ...
    if isempty(trigs)
        warning('No triggers found for %s. Skipping.', trial);
        continue;
    end

    % Artifact removal with extracted trigger info
    tic;
    [cleaned_all, stim_drift_all] = ...
        compare_template_modes(trial_path, updated_params, template_modes, ...
        trigs, repeat_boundaries);
    toc;
    artifact_removed_data = squeeze(cleaned_all(:, 1, :));  % [nChans x time]
    
    % Save cleaned data
    save_path_trig = fullfile(trial_path, 'trig_info.mat');
    save(save_path_trig, 'trigs', 'stim_drift_all', 'repeat_boundaries');
    clear trigs stim_drift_all repeat_boundaries;
    fprintf('Saved trig info for %s\n', trial);

    save_path_corrected = fullfile(trial_path, 'neural_data_artifact_removed.mat');
    save(save_path_corrected, 'artifact_removed_data', '-v7.3');
