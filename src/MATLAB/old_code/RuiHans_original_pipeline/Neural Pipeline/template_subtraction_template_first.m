function amplifier_data_cleaned = template_subtraction(amplifier_data, trigs, chan, params, template_mode, plot_chan)
...
%% === Subtract Templates First ===
amplifier_data_cleaned = amplifier_data;
for i = 1:NSTIM
    segment = trigs_beg(i):trigs_end(i);
    amplifier_data_cleaned(segment) = amplifier_data_cleaned(segment) - template(i, :);
end
%% === Fit Block-Level Decay and Remove ===
lpf_cutoff = 100;  % Hz
[b_lpf, a_lpf] = butter(2, lpf_cutoff / (fs/2), 'low');
fit_duration_ms = 1500;
fit_samples = round(fit_duration_ms * fs / 1000);

for r = 1:num_repeats
    idx = (repeat_boundaries(r)+1):repeat_boundaries(r+1);
    trig_range = trigs_beg(idx);
    full_range_start = trig_range(1) - buffer;
    range_start = full_range_start + 40;
    if r < num_repeats
        range_end = trigs_beg(repeat_boundaries(r+2)) - 1;
    else
        range_end = trig_range(end) + buffer;
    end

    if range_start > 0 && range_end <= length(amplifier_data)
        raw_block_original = double(amplifier_data(full_range_start:range_end));  % Before any subtraction
        raw_block = double(amplifier_data_cleaned(full_range_start:range_end));   % After template subtraction
        fit_start_idx = range_start - full_range_start + 1;
        fit_end_idx = fit_start_idx + fit_samples - 1;
        fit_window = raw_block(fit_start_idx:min(fit_end_idx, numel(raw_block)));
        smooth_block = filtfilt(b_lpf, a_lpf, fit_window);

        fit_x = (range_start:range_start+length(smooth_block)-1) - full_range_start;
        fit_y = smooth_block;

        % === Composite Function: overshoot + damped oscillation + exp decay ===
        fit_fun_shape = @(p, x) ...
            p(1) * exp(-p(2) * x / fs) .* cos(2 * pi * p(3) * x / fs + p(4)) + ...
            p(5) * exp(-p(6) * x / fs) + ...
            p(7) * exp(-p(8) * x / fs);

        % === Wrapper that enforces start/end match ===
        fit_fun = @(p, x) rescale_fit_to_bounds(fit_fun_shape(p, x), fit_y(1), fit_y(end));

        A1 = max(fit_y) - min(fit_y); b1 = 10; f0 = 60; phi0 = 0;
        A2 = A1 / 2; b2 = 2; A3 = -A1 / 3; b3 = 20; C0 = mean(fit_y(end-10:end));
        p0 = [A1, b1, f0, phi0, A2, b2, A3, b3];
        lb = [0,  0, 50, -2*pi, -Inf,  0, -Inf, 0];
        ub = [Inf, 100, 70,  2*pi,  Inf, 20,    0, 100];


        opts = optimset('Display', 'off');
        try
            p_fit = lsqcurvefit(fit_fun, p0, fit_x, fit_y, lb, ub, opts);
            x_full = (range_start:range_end) - range_start;
            decay_core = fit_fun(p_fit, x_full);
            tail_mean = mean(decay_core(end-4:end));
            decay_core = decay_core - tail_mean;
            decay_fit = [zeros(1, range_start - full_range_start), decay_core];
            amplifier_data_cleaned(full_range_start:range_end) = ...
                amplifier_data_cleaned(full_range_start:range_end) - decay_fit;

            % === Store first block info for plotting ===
            if r == 1 && chan == plot_chan
                p_fit_1 = p_fit;
                fit_x_1 = fit_x;
                fit_y_1 = fit_y;
                full_range_start_1 = full_range_start;
                range_start_1 = range_start;
                range_end_1 = range_end;
                fit_fun_1 = fit_fun;
                amplifier_data_pre_decay = raw_block;                % cleaned
                amplifier_data_pre_subtraction = raw_block_original; % raw
            end
        catch
            warning('Fit failed for block %d', r);
        end
...
if exist('p_fit_1', 'var') && chan == plot_chan
    fs = 30000;
    t_ms = (full_range_start_1:range_end_1) / fs * 1000;
    t_fit_ms = (fit_x_1 + full_range_start_1 - 1) / fs * 1000;

    decay_fit = fit_fun_1(p_fit_1, fit_x_1);
    filtered_block = amplifier_data_cleaned(full_range_start_1:range_end_1);

    figure('Name', sprintf('Decay Fit - Switched Mode (Chan %d)', chan), ...
        'Position', [100 100 1000 400]); hold on;

    plot(t_ms, amplifier_data_pre_subtraction, 'Color', [0.5 0.5 0.5 0.5], ...
        'DisplayName', 'Raw Before Template Subtraction');
    plot(t_ms, amplifier_data_pre_decay, 'Color', [0.8 0.3 0.3 0.4], ...
        'DisplayName', 'Raw After Template Subtraction');
    plot(t_fit_ms, fit_y_1, 'Color', [0.1 0.6 0.1 0.7], ...
        'LineWidth', 1.2, 'DisplayName', 'Low-pass Fit Input');
    plot(t_fit_ms, decay_fit, 'k-', 'DisplayName', 'Decay Fit');
    plot(t_ms, filtered_block, 'Color', [0.2 0.6 0.9], ...
        'DisplayName', 'Filtered Output');
    xlabel('Time (ms)'); ylabel('Amplitude');
    title(sprintf('Decay Fit After Template Subtraction (Chan %d)', chan));
