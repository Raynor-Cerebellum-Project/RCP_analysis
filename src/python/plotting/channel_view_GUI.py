...
def load_ana():
    return [
        se.read_split_intan_files(folder_path=f, mode="concatenate",
                                  stream_name=TARGET_STREAM, use_names_as_ids=True)
...
@timed("load digital")
def load_dig():
    return se.read_split_intan_files(folder_path=SESSION_FOLDERS[TRIAL_IDX],
                                     mode="concatenate",
...
if aligned_ok:
    ...
    def stim_on_window(abs_start, abs_end):
        ...
        return np.interp(s_hat, np.arange(stim_src.size, dtype=float), stim_src, left=0.0, right=0.0)
else:
...
hb = Heartbeat("get_traces", interval=5); hb.start()
t0 = time.perf_counter()
...
ax.set_title(f"rec_pp around stim #{STIM_NUM} (trial {TRIAL_IDX}) — "
