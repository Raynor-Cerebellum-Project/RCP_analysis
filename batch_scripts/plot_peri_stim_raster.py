...
def _get_raster_data(peak_times_ms, events_ms, win_ms):
    """
...
def plot_channel_group(ax_raster, ax_psth, 
    ...
    # Stimulation shading
    if stim_dur_ms > 0:
        ax_raster.axvspan(0, stim_dur_ms, color='gray', alpha=0.3, edgecolor=None)
    
    # Formatting
    ax_raster.set_xlim(WIN_PLOT_MS)
    ...
    # Stimulation shading
    if stim_dur_ms > 0:
        ax_psth.axvspan(0, stim_dur_ms, color='gray', alpha=0.3, edgecolor=None)
    
    ax_psth.set_xlim(WIN_PLOT_MS)
...
def process_file(npz_path):
    ...
    if n_trials_ref == 0:
        print(f"Skipping {npz_path.name}: No events and no trial counts found.")
        return
    
    # Use empty list if no events, but proceed
    if len(event_ms) == 0:
        # print(f"Note: No event timestamps in {npz_path.name}. Raster will be empty, PSTH only.")
        pass # Silent
    ...
    if np.size(stim_list) > 0:
        ...
        try:
            ...
            stim_chs_str = f" | Stim Chs: {list(sl)}"
        except:
    ...
    if 'NPRW_peak_ms_dedup' in data and data['NPRW_peak_ms_dedup'].item():
        ...
        counts_data = data['NPRW_counts'] # (n_trials, n_channels, n_bins)
        edges_ms = data['NPRW_edges_ms']
        
        # Load bin centers
        centers_ms = data['NPRW_rel_t'] if 'NPRW_rel_t' in data else None
        ...
        for i, ch in enumerate(tqdm(sorted_ch_ids, desc="NPRW Chs", leave=False)):
            if i >= n_ch: break
            ...
            # Subplot indices (Raster on top, PSTH on bottom)
            ...
            plot_channel_group(ax_raster, ax_psth, peak_times, event_ms, binned, edges_ms, f"Ch {ch}", stim_dur_ms=stim_dur_ms, bin_centers=centers_ms, n_trials_ref=n_trials_ref)

            
            # Only show bottom x-labels for the very last row
            if 2*r+1 < (rows*2 - 2):
        ...
        if len(stim_chs_str) > 0:
             plt.subplots_adjust(top=0.90) # Make room for big title
        
        # Save
        out_rel = npz_path.parent.relative_to(PERI_ROOT)
        save_dir = FIG_ROOT / out_rel
        save_dir.mkdir(parents=True, exist_ok=True)
        
        fig_name = f"[processed]_Cond{br_idx:03d}_{cond_type}_NPRW.png"
    ...
    if has_ua and 'UA_peak_ms_dedup' in data and data['UA_peak_ms_dedup'].item():
        ...
        counts_data = data['UA_counts']
        edges_ms = data['UA_edges_ms']
        
        # Load bin centers
        centers_ms = data['UA_rel_t'] if 'UA_rel_t' in data else None
        ...
        def get_region_priority(reg_name):
            ...
            for idx, key in enumerate(REGION_ORDER):
                if key.lower() == r_lower:
                    return idx
            return 999 # Unknown/Other at the end
        ...
        # Sort regions first by priority
        all_regions = sorted(regions.keys(), key=lambda r: (get_region_priority(r), r))
        
        for reg in all_regions:
            # Sort within region by channel ID usually best
            ch_list = regions[reg]
            # Try sorting by int(ch_id)
            try:
            ...
            for item in ch_list:
                # item is (ch, linear_idx, real_id)
                sorted_by_region.append((reg, item[0], item[1], item[2]))
        
        for i, (reg, ch_id, linear_idx, real_id) in enumerate(tqdm(sorted_by_region, desc="UA Chs", leave=False)):
            r = (i // cols)
            c = (i % cols)
            ...
            plot_channel_group(ax_raster, ax_psth, peak_times, event_ms, binned, edges_ms, f"{reg} - Ch {real_id}", stim_dur_ms=stim_dur_ms, bin_centers=centers_ms, n_trials_ref=n_trials_ref)
        ...
        fig_name = f"[processed]_Cond{br_idx:03d}_{cond_type}_UA.png"
        fig_ua.savefig(save_dir / fig_name, dpi=100, bbox_inches='tight')
        plt.close(fig_ua)
        
    print(f"Finished {npz_path.name}")
...
def main():
    ...
    Parallel(n_jobs=8)(delayed(process_file)(f) for f in tqdm(files, desc="Files"))
