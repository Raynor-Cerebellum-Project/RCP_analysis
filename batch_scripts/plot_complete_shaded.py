...
# ---------- figures ----------
FIG_ROOT   = OUT_BASE / "figures/shaded"; FIG_ROOT.mkdir(parents=True, exist_ok=True)
...
def main():
    ...
    # SEARCH IN TARGET FOLDERS
    target_dirs = [PERI_ROOT / "Target_A", PERI_ROOT / "Target_B"]
    
    # Collect all npz files from both folders
    files = []
    ...
    for peri_stim_npz_loc in files:
        ...
        print(f"Processing {peri_stim_npz_loc.name}...")

        with np.load(peri_stim_npz_loc, allow_pickle=True) as peri_stim_npz:
            # metadata from inside the file
            ...
            # beh_rel_t_v        = peri_stim_npz["beh_rel_t_v"] # unused
            cam0_pos_med       = peri_stim_npz["beh_cam0_pos_med"]
            cam1_pos_med       = peri_stim_npz["beh_cam1_pos_med"]
            cam0_vel_med       = peri_stim_npz["beh_cam0_vel_med"]
            cam1_vel_med       = peri_stim_npz["beh_cam1_vel_med"]
            # per-trial baseline-zeroed segments: (n_trials, D, T)
            cam0_pos_segs      = peri_stim_npz["beh_cam0_segs"] if "beh_cam0_segs" in peri_stim_npz.files else np.zeros((0,0,0), float)
            cam1_pos_segs      = peri_stim_npz["beh_cam1_segs"] if "beh_cam1_segs" in peri_stim_npz.files else np.zeros((0,0,0), float)
            ...
            # Var usually shares the same time axis, or check for separate var key if your extract script saves it
            # extract_peri_stim saves "NPRW_rel_t" for var too usually, but let's check what was saved
            # Checking extract_peri_stim.py: it saves "NPRW_rel_t" and "UA_rel_t" once.
        ...
        # Determine subfolder for output based on input parent
        # e.g. "Target A" or "Target B"
        parent_name = peri_stim_npz_loc.parent.name
        out_subdir_med = FIG.peri_posvel_median / parent_name
        out_subdir_med.mkdir(parents=True, exist_ok=True)
