...
%     Remove the 'd' from the file name to denote that file is now calibrated:

      nameout = [nameinfirst(1:length(nameinfirst)-5) '.' sessionname];

      disp(['loading ' namein ' ...'])
      eval(['load ' namein ' -mat'])

      eval(lower(namenew(1:length(namenew)-5)))
