...
in1to4 = []; out1 = []; model = ['---';'---';'---';'---';'---'];
in1to4 = zeros(length(ghpos),4);
out1 = zeros(length(ghpos),1);
...
in1 = uicontrol('Parent',h0, ...
    ...
    'Callback','k = []; sig = []; k = get(in1, ''value''); if (k == 1), in1to4(:,1) = 0; model(1,:) = ''---''; else, lat_t = lat; lat = 0; sig = eval(Signaldefinitions(listfind((Plotlist(k-1,:)), Signals),:)); if ( (Plotlist(k-1,3) == ''v'') | (Plotlist(k-1,1:3) == ''vcj'') | (Plotlist(k-1,1:3) == ''vvg'') ); in1to4(2:length(sig)+1,1) = sig; else; in1to4(1:length(sig),1) = sig; end; lat = lat_t; model(1,:) = (Plotlist(k-1,1:3)); end;', ...
    'Units','points', ...
    'ListboxTop',0, ...
...
in3 = uicontrol('Parent',h0, ...
    'String',['none|' makelist1(Plotlist,NoofDisplayed)], ...
    'Callback','l = []; sig = []; l = get(in3, ''value''); if (l == 1), in1to4(:,3) = 0; model(2,:) = ''---''; else, lat_t = lat; lat = 0; sig = eval(Signaldefinitions(listfind((Plotlist(l-1,:)), Signals),:)); if ( (Plotlist(l-1,3) == ''v'') | (Plotlist(l-1,1:3) == ''vcj'') | (Plotlist(l-1,1:3) == ''vvg'') ); in1to4(2:length(sig)+1,3) = sig; else; in1to4(1:length(sig),3) = sig; end; lat = lat_t; model(3,:) = (Plotlist(l-1,1:3)); end;', ...
    'Units','points', ...
    'ListboxTop',0, ...
...
outs = uicontrol('Parent',h0, ...
    'String',['none|' makelist1(Plotlist,NoofDisplayed)], ...
    'Callback','m = []; sig = []; m = get(outs, ''value''); if (m == 1), out1(:,1) = 0; model(5,:) = ''---''; else, lat_t = lat; lat = 0; sig = eval(Signaldefinitions(listfind((Plotlist(m-1,:)), Signals),:)); out1(1:length(sig),1) = sig; lat = lat_t; model(5,:) = (Plotlist(m-1,1:3)); if ((Plotlist(m-1,1:3) ~= '' fr'')); mode_flag = 1234; end; end;', ...
    'Units','points', ...
    'ListboxTop',0, ...
...
in2d = uicontrol('Parent',h0, ...
    'String','Derivative', ...
    'Callback','if (sum(in1to4(:,2)) == 0); in1to4(2:length(in1to4),2) = diff(in1to4(:,1))./0.001; model(2,:) = ''der''; else; in1to4(1:length(in1to4),2) = 0; model(2,:) = ''---''; end;', ...
    'Units','points', ...
    'ListboxTop',0, ...
...
in4d = uicontrol('Parent',h0, ...
    'String','Derivative', ...
    'Callback','if (sum(in1to4(:,4)) == 0); in1to4(2:length(in1to4),4) = diff(in1to4(:,3))./0.001; model(4,:) = ''der''; else; in1to4(1:length(in1to4),4) = 0; model(4,:) = ''---''; end;', ...
    'Units','points', ...
    'ListboxTop',0, ...
...
in2a = uicontrol('Parent',h0, ...
    'String','Amplitude', ...
    'Callback','if (sum(in1to4(:,2)) == 0); for j = 1:size(M,1); in1to4(M(j,1):M(j,2),2) = in1to4(M(j,2)+lat,1) - in1to4(M(j,1)+lat,1); end; model(2,:) = ''amp''; else; in1to4(1:length(in1to4),2) = 0; model(2,:) = ''---''; end;', ...
    'Units','points', ...
    'ListboxTop',0, ...
...
in4a = uicontrol('Parent',h0, ...
    'String','Amplitude', ...
    'Callback','if (sum(in1to4(:,4)) == 0); for j = 1:size(M,1); in1to4(M(j,1):M(j,2),4) = in1to4(M(j,2)+lat,3) - in1to4(M(j,1)+lat,3); end; model(4,:) = ''amp'';; else; in1to4(1:length(in1to4),4) = 0; model(4,:) = ''---''; end;', ...
    'Units','points', ...
    'ListboxTop',0, ...
...
ig1 = uicontrol('Parent',h0, ...
    'String','Ignore Input', ...
    'Callback','if (sum(in1to4(:,1)) ~= 0); in1to4(1:length(in1to4),1) = 0; model(1,:) = ''---''; else; k = []; sig = []; k = get(in1, ''value''); if (k == 1), in1to4(:,1) = 0; model(1,:) = ''---''; else, lat_t = lat; lat = 0; sig = eval(Signaldefinitions(listfind((Plotlist(k-1,:)), Signals),:)); in1to4(1:length(sig),1) = sig; lat = lat_t; end; model(1,:) = (Plotlist(k-1,1:3)); end;', ...
    'Units','points', ...
    'ListboxTop',0, ...
...
ig3 = uicontrol('Parent',h0, ...
    'String','Ignore Input', ...
    'Callback','if (sum(in1to4(:,3)) ~= 0); in1to4(1:length(in1to4),3) = 0; model(3,:) = ''---''; else; k = []; sig = []; k = get(in3, ''value''); if (k == 1), in1to4(:,3) = 0; model(3,:) = ''---''; else, lat_t = lat; lat = 0; sig = eval(Signaldefinitions(listfind((Plotlist(k-1,:)), Signals),:)); in1to4(1:length(sig),3) = sig; lat = lat_t; end; model(3,:) = (Plotlist(k-1,1:3)); end;', ...
    'Units','points', ...
    'ListboxTop',0, ...
...
in1a = uicontrol('Parent',h0, ...
    'String',['none|' makelist1(Plotlist,NoofDisplayed)], ...
    'Callback','k = []; sig = []; k = get(in1a, ''value''); if (k == 1), in1to4(:,2) = 0; model(2,:) = ''---''; else, lat_t = lat; lat = 0; sig = eval(Signaldefinitions(listfind((Plotlist(k-1,:)), Signals),:)); if ( (Plotlist(k-1,3) == ''v'') | (Plotlist(k-1,1:3) == ''vcj'') | (Plotlist(k-1,1:3) == ''vvg'') ); in1to4(2:length(sig)+1,2) = sig; else; in1to4(1:length(sig),2) = sig; end; lat = lat_t; model(2,:) = (Plotlist(k-1,1:3)); end;', ...
    'Units','points', ...
    'ListboxTop',0, ...
...
in3a = uicontrol('Parent',h0, ...
    'String',['none|' makelist1(Plotlist,NoofDisplayed)], ...
    'Callback','l = []; sig = []; l = get(in3a, ''value''); if (l == 1), in1to4(:,4) = 0; model(4,:) = ''---''; else, lat_t = lat; lat = 0; sig = eval(Signaldefinitions(listfind((Plotlist(l-1,:)), Signals),:)); if ( (Plotlist(l-1,3) == ''v'') | (Plotlist(l-1,1:3) == ''vcj'') | (Plotlist(l-1,1:3) == ''vvg'') ); in1to4(2:length(sig)+1,4) = sig; else; in1to4(1:length(sig),4) = sig; end; lat = lat_t; model(4,:) = (Plotlist(l-1,1:3)); end;', ...
    'Units','points', ...
    'ListboxTop',0, ...
...
h1 = uicontrol('Parent',h0, ...
    'String','Estimate', ...
    'Callback','clear ho h1 in1 in2 in3 in4 outs x se error_sim VAF BIC est Others; close; [x,se,error_sim,VAF,BIC,est,Others] = dodyn3(M,ns,lat,out1,in1to4,mode_flag); plotdyn', ...
    'Fontsize',15, ...
    'Units','points', ...
    'ListboxTop',0, ...
...
h1 = uicontrol('Parent',h0, ...
    'String','Get Latency', ...
    'Callback','clear ho h1 in1 in2 in3 in4 outs sig; close; dynlatpick; clear filename_e out1 in1to4 model; ', ...
    'Fontsize',9, ...
    'Units','points', ...
    'ListboxTop',0, ...
...
h1 = uicontrol('Parent',h0, ...
    'String','Bootstrap', ...
    'Callback','clear ho h1 in1 in2 in3 in4 outs sig filename_m; close; clc; if exist(''hdatadisplay''); set(hdatadisplay,''visible'',''on''); end; global abort_flag; [filename_m] = bootstrap(M,ns,lat,out1,in1to4,mode_flag,userdir,model,0); set(hdatadisplay,''visible'',''on''); clear out1 in1to4 model; ', ...
    'Fontsize',9, ...
    'Units','points', ...
    'ListboxTop',0, ...
