function [x,se,error_sim,VAF,BIC,est,Others] = sordodyn3(M,ns,lat,out1,in1to4,mode_flag)
...
if (pred_flag ~= 1) & (out1(1) ~= 9999),
    ...
    [x,RESNORM,RESIDUAL,EXITFLAG] = lsqnonlin('sorerrmdyn2',x0,[],[],options,t_out,t_in);  
    Others.exit = EXITFLAG;
    
%    x = [([ones(length(t_in),1) t_in] \ t_out)'];  %Jay's code, see bottom
    est = x(1) + x(2).*t_in(:,2) + x(3).*(t_in(:,2)).^2 + x(3).*(t_in(:,2)).^3 + x(3).*(t_in(:,2)).^4; 
...
elseif (pred_flag ~= 1),
    ...
    [x,RESNORM,RESIDUAL,EXITFLAG] = lsqnonlin('sorerrmdyn2nb',x0,[],[],options,t_out,t_in);  
    Others.exit = EXITFLAG;
    ...
    est = x(1) + x(2).*t_in(:,2) + x(3).*(t_in(:,2)).^2 + x(4).*(t_in(:,2)).^3 + x(5).*(t_in(:,2)).^4; 
    se = sd_err(t_out,t_in,est,Others); %sub-function
    Others.exit = 111111;
    Others.x0 = x0;
