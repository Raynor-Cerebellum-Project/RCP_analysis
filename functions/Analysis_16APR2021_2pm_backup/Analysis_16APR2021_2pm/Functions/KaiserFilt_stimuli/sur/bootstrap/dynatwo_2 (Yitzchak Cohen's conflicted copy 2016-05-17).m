...
in1 = uicontrol('Parent',h0, ...
    ...
    'Callback','k = []; sig = []; k = get(in1, ''value''); if (k == 1), in1to4(:,1) = 0; model(1,1:3) = ''---'';model(1,4:end)='' ''; else, lat_t = lat; lat = 0; sig = eval(Signaldefinitions(listfind((Plotlist(k-1,:)), Signals),:)); in1to4(1:length(sig),1) = sig; lat = lat_t; model(1,1:length(Plotlist(k-1,1:end))) = (Plotlist(k-1,1:end));; end; set(in2d, ''Value'', 0); set(in2a, ''Value'', 0);set(ig1, ''Value'', 0);', ...
    'Units','points', ...
    'ListboxTop',0, ...
...
outs = uicontrol('Parent',h0, ...
    'String',['none|' makelist1(Plotlist,NoofDisplayed)], ...
    'Callback','m = []; sig = []; m = get(outs, ''value''); if (m == 1), out1(:,1) = 0; model(5,1:3) = ''---''; else, lat_t = lat; lat = 0; sig = eval(Signaldefinitions(listfind((Plotlist(m-1,:)), Signals),:)); out1(1:length(sig),1) = sig; lat = lat_t; model(5,1:length(Plotlist(m-1,1:end))) = (Plotlist(m-1,1:end)); if (~strcmp(Plotlist(m-1,1:end),''fr'')); mode_flag = 1234; end; end;set(ig5, ''Value'', 0);', ...
    'Units','points', ...
    'ListboxTop',0, ...
...
in1a = uicontrol('Parent',h0, ...
    'String',['none|' makelist1(Plotlist,NoofDisplayed)], ...
    'Callback','k = []; sig = []; k = get(in1a, ''value''); if (k == 1), in1to4(:,2) = 0; model(2,:) = ''---''; else, lat_t = lat; lat = 0; sig = eval(Signaldefinitions(listfind((Plotlist(k-1,:)), Signals),:)); in1to4(1:length(sig),2) = sig; lat = lat_t; model(2,:) = (Plotlist(k-1,1:end)); end;', ...
    'Units','points', ...
    'ListboxTop',0, ...
...
in3a = uicontrol('Parent',h0, ...
    'String',['none|' makelist1(Plotlist,NoofDisplayed)], ...
    'Callback','l = []; sig = []; l = get(in3a, ''value''); if (l == 1), in1to4(:,4) = 0; model(4,:) = ''---''; else, lat_t = lat; lat = 0; sig = eval(Signaldefinitions(listfind((Plotlist(l-1,:)), Signals),:)); in1to4(1:length(sig),4) = sig; lat = lat_t; model(4,:) = (Plotlist(l-1,1:end)); end;', ...
    'Units','points', ...
    'ListboxTop',0, ...
...
h1 = uicontrol('Parent',h0, ...
    'String','Get Latency', ...
    'Callback','clear ho h1 in1 in2 in3 in4 outs sig; close; [x,se,error_sim,VAF,BIC,est,Others] = dynlatpick(M,ns,lat,out1,in1to4,mode_flag); M=M+lat-Others.lat;lat=Others.lat; plotdyn_2; clear filename_e out1 in1to4 model; ', ...
    'Fontsize',9, ...
    'Units','points', ...
    'ListboxTop',0, ...
...
h1 = uicontrol('Parent',h0, ...
    'String','Latency Range:', ...
    'Units','points', ...
    'ListboxTop',0, ...
    'Position',[213.75 80 71.25 13.5], ...
    'Style','text', ...
    'Tag','StaticText1');
    
lat_max=20;
lat_range = uicontrol('Parent', h0,...
    'String', lat_max,...
    'Units','points', ...
    'ListboxTop',0, ...
    'Position',[213.75 63 71.25 13.5], ...
    'Style','edit', ...
    'Tag','latrange',...
    'Callback', 'lat_max = str2double(get(lat_range, ''String''));');    
