...
def load_events(session_name):
    # Standardized loading from consolidated event extraction
    ...
    try:
        ...
        print(f"[warn] Failed to load events {path}: {e}")
        return None

def filter_stims_by_reach(stim_ms, events, target_type, window_sec=0.2):
    """
    Keep stims where a reach of 'target_type' ('A' or 'B') starts within window_sec after stim.
    """
    if events is None:
        return np.array([])
        
    ev_starts = events['start_sec']
    ev_types  = events['type']
    
    # Filter events to relevant type
    relevant_starts = ev_starts[ev_types == target_type]
    
    keep_mask = np.zeros(len(stim_ms), dtype=bool)
    
    # Vectorized or iterative check? Iterative is safe enough for N=100s
    for i, t_stim in enumerate(stim_ms):
        t_stim_sec = t_stim / 1000.0
        # Check if any event in [t_stim, t_stim + window]
        # Using searchsorted for speed if sorted, typically events are sorted
        # events must be sorted by start time
        
        # Simple window check:
        # valid = (relevant_starts > t_stim_sec) & (relevant_starts < t_stim_sec + window_sec)
        # Using a slightly wider check for strictness? User said "within 200ms".
        
        valid = (relevant_starts >= t_stim_sec) & (relevant_starts <= t_stim_sec + window_sec)
        if valid.any():
            keep_mask[i] = True
            
    return stim_ms[keep_mask]
...
def main():
    ...
    for file in files:
        ...
        # Robust Session Name Parsing for Event Loading
        # 1. Trust meta if present and sane?
        # 2. Parse filename features (Intan/BR indices) to reconstruction standardized name.
        
        # Strategy: Try to reconstruct from filename first as it aligns with event filenames
        # Pattern: aligned_{Subject}_{Date}_{Time}_..._Intan_{idx}_...
        try:
            parts = [p for p in file.stem.split('_') if p]
            if 'Intan' in parts:
                idx_intan = parts.index('Intan')
                idx_val = parts[idx_intan + 1]
                # Subject is everything after 'aligned' (0) and before Date/Time (index-2)
                # This assumes standard naming: aligned_Subj_Date_Time_Intan_...
                subj_parts = parts[1 : idx_intan - 2]
                subj_str = "_".join(subj_parts)
                sess_for_events = f"{subj_str}_{idx_val}"
            else:
                 # Fallback: remove aligned_ prefix, handle double underscores
                 sess_for_events = file.stem.replace("aligned_", "").replace("__", "_")
        except:
             sess_for_events = meta.get("session", file.stem)

        events = load_events(sess_for_events)
        if events is None:
            # First fallback failed, try raw sess from meta
            if sess != sess_for_events:
                 events = load_events(sess)
        
        if events is None:
             print(f"[warn] Could not load events for {sess_for_events} (or {sess})")
             # We can continue but filtering will return empty
             # continue

        # ---- Loop A, B Conditions ----
        for target_type in ['A', 'B']:
            print(f"  > Processing Reach Target: {target_type}")
            
            stim_ms = filter_stims_by_reach(stim_ms_full, events, target_type, window_sec=0.2)
            n_stims = len(stim_ms)
            if n_stims == 0:
                print(f"    [SKIP] No stims followed by Reach {target_type}")
                continue
            
            suffix = f"_Reach{target_type}"
            print(f"    Found {n_stims} trials for {target_type}")
            ...
            # ---------------- POSITION plot ----------------
            out_svg_posvel = FIG.peri_posvel / f"Cond_{br_idx}__peri_stim__posvel{suffix}.png"
            ...
            rcp.stacked_heatmaps_plus_behv(
                ...
                sess=sess,
                overall_title=f"{overall_title} | Reach {target_type} (n={n_stims})",
            )
            # --- Variance fig ---
            ...
            out_svg_posvel_var = FIG.peri_posvel / f"Cond_{br_idx}__peri_stim__posvel_VAR{suffix}.png"
