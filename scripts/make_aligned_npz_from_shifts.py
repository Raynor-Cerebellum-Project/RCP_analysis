    ...
    for row in rows:
        try:
            ...
            np.savez_compressed(
                ...
                intan_t_ms=intan_t_ms.astype(np.float64),  # original, for reference
                intan_t_ms_aligned=common_t.astype(np.float64),
                ...
                ua_t_ms=ua_t_ms.astype(np.float64),  # original, for reference
                ua_t_ms_aligned=common_t.astype(np.float64),
                ...
                # Stim (absolute Intan ms; convert later using anchor_ms)
