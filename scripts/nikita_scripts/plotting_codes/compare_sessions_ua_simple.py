...
# Raw trace settings
TRACE_WINDOW_MS = (-600, 600)  # Window for raw traces relative to movement onset
...
def run_analysis(cfg: ComparisonConfig):
    ...
    for spec in cfg.session_specs:
        ...
        ctrl_order = build_channel_order(ctrl_data, elec_mapping, cfg.time_windows)
        stim_order = build_channel_order(stim_data, elec_mapping, cfg.time_windows)
        
        for region in REGION_ORDER:
            print(f"    {region}: {len(ctrl_order.get(region, []))} channels")
