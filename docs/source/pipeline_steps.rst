...
#. **Frame correction** — ``preprocessing/OCR_frame_correction.py``
#. **DeepLabCut 2-camera data alignment** — ``batch_scripts/align_dlc_two_cams_to_br.py``
#. **VOG to Blackrock (BR) alignment** — ``batch_scripts/align_VOG_to_br.py``
...
#. **UA preprocessing and spike detection** (subspace) —
   ...
   * Alternatively use ``batch_scripts/UA_BR_analysis_mf.py`` for a matched filter.

#. **Create aligned files** — ``batch_scripts/make_aligned_npz_and_mat.py``
...
#. **Kinematics analysis** — ``batch_scripts/plot_plateau_analysis.py``
