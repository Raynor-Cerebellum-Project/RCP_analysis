...
disp([Err_mean(1,1), Err_mean(1,2)]);
...
xlabel('Stimulation Side');
ylabel('Absolute Endpoint Error (deg)');
title('Endpoint Error by Stimulation Side (Baseline)');
box off; set(gca, 'TickDir', 'out');
%
%If you want to plot a segment labelled "raw_example"
% figure;
...
xlabel('Stimulation Side');
ylabel('Absolute Endpoint Error (deg)');
title('Comparison of Endpoint Error by Stimulation Side');
legend(b, {'Baseline', 'Stim (64 channels, 400 Hz, 100 ms)'});
box off; set(gca, 'TickDir', 'out');