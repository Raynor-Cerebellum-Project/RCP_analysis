...
# =========================
dlc_csv = Path("/home/bryan/mnt/cullen/Current Project Databases - NHP/2025 Cerebellum prosthesis/Bryan/data/Bert_reaching_001/Video_DLC/videos/CamVideo_006_Cam-0DLC_Resnet50_Bert_reach_pilotAug6shuffle1_snapshot_010.csv")
blackrock_dir = Path("/home/bryan/mnt/cullen/Current Project Databases - NHP/2025 Cerebellum prosthesis/Bryan/data/Bert_reaching_001/Blackrock")
SYNC_CHANNEL_ID = 142
...
if "CamVideo_006" in dlc_csv.name:
    ...
    raise ValueError("Unknown video number in filename; update logic to match the correct Blackrock file.")
...
if len(ns5_files) <= blackrock_file_index:
    raise IndexError(f"Only {len(ns5_files)} .ns5 files found; index {blackrock_file_index} is out of range.")
...
if SYNC_CHANNEL_ID not in ch_ids:
    raise ValueError(f"Sync channel {SYNC_CHANNEL_ID} not found in file. Available: {ch_ids}")
...
# =========================
sync_preview = rec.get_traces(channel_ids=[SYNC_CHANNEL_ID], start_frame=0, end_frame=int(sf * SHOW_FIRST_SECONDS)).astype(float).squeeze()
