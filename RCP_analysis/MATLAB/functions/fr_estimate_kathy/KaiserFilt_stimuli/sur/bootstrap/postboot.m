...
fprintf(fid,'Perc. conv.: \t\t %0.4f\n', convperc);
%fprintf(fid,'\n\n\n');

fclose(fid); 

%% Added by AD, February 2012

%select only relevant columns
cols = [1 2 3 4 5 11 12];

%collect mean parameter estimates
meanparams = mean(DATA,1);
parametersForAnalysis = meanparams(cols);

%assess confidence intervals, 
%make parameters with CI overlapping zero = 0
for col = 2:5
    if CI_BCa(1,col)/CI_BCa(2,col) < 1
        parametersForAnalysis(col) = 0;
    end
end
...
% % % % If latency was also optimizesd!
% % % if  (size(mode_flag,2) == 1) | ((size(mode_flag,2) > 1) & (mode_flag(2) == 5678)),
% % %     
% % %     summary_lat = figure('position',[96   243   889   435],'name','Summary diagram: LATENCY','PaperOrientation', 'landscape', 'PaperPosition' , [0.25 0.25 10.5 8]);
% % %     
% % %     q = 20;
% % %     
% % %     hist(DATA(1:n,q),50);
...
% % %     yrange = get(gca,'ylim');
% % %     
% % %     p_mean = line([ORIG(q) ORIG(q)],yrange);
% % %     set(p_mean,'color',[1 0 0],'LineStyle','-')
% % %     p_boot= line([CI_BCa(1,q) CI_BCa(2,q)],[-15 -15]);
% % %     set(p_boot,'color',[0 0 1],'LineStyle','-','linewidth',10)
% % %     
% % %     axis tight
% % %     axis([xlim -40 max(ylim)]) 
...
% % %     disp('Latency Information: (mean, LB, RB)')
% % %     disp(' ')
% % %     disp(sprintf('  %0.4g\t %0.4g\t %0.4g\n', ORIG(q) , CI_BCa(1,q) , CI_BCa(2,q)));
% % % 
% % %     
% % % end
% % % 
...
% filename_lat = [filename_m(1:length(filename_m)-4) '_lat.fig'];
...
% saveas(summary_lat,filename_lat)
% 
% if (print_flag == 1)
%     print(diag_p)
%     print(param_p)
