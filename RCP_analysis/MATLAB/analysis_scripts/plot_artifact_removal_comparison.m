% === Load data from disk ===
trial_path = '/Volumes/cullenlab_server/Current Project Databases - NHP/2025 Cerebellum prosthesis/Bryan/Data/BL_RW_003_Session_1/Intan/BL_closed_loop_STIM_003_250528_145904';
...
if ~isempty(stim_drift_all) && size(stim_drift_all, 1) >= plot_chan && ...
    ...
    for r = 1:numel(drift)
        if isfield(drift(r), 'range') && ~isempty(drift(r).range)
            range = drift(r).range;
            idx1 = range(1);
            idx2 = range(2);
            values = drift(r).values;
            if idx2 <= length(drift_trace) && length(values) == (idx2 - idx1 + 1)
                drift_trace(idx1:idx2) = values(:)';
            end
        end
    end
...
plot(time_ms, cleaned_trace, ...
    ...
    'DisplayName', 'Cleaned');
xlim([4 30] * 1000);  % Convert to ms since time_ms is in milliseconds
