...
session = 'Bert/BL_RW_003_Session_1';
...
switch session
    ...
    case 'Nike/20260326_NRR_RW027_fastig'
        ...
        % 1-6 weird
        baseline_file_nums = 14;
        trial_indices = [8:9, 13];
        segment_fields_random = {'ipsi_nan' , 'contra_nan', 'ipsi_0' , 'contra_0', 'ipsi_100' , 'contra_100', 'ipsi_200' , 'contra_200'};
        segment_fields = {'ipsi' , 'contra'};
    case 'Nike/20260423_NRR_RW033_fastig'
        EndPoint_pos = 30; EndPoint_neg = -30;
        % 1-9 segmented
        baseline_file_nums = 1;
...
% Resolve side labels for this session
if contains(session, 'Bert')
    ipsi_label   = 'active_like_stim_neg';
    contra_label = 'active_like_stim_pos';
else
    ipsi_label   = 'ipsi';
    contra_label = 'contra';
end
...
raw_metrics_all = cell(height(T), 1);
...
ipsi_fields   = all_fields(contains(all_fields, ipsi_label));
contra_fields = all_fields(contains(all_fields, contra_label));
...
combine_map = struct( ...
    'ipsi',   {{ipsi_label,   [ipsi_label,   '_nan']}}, ...
    'contra', {{contra_label, [contra_label, '_nan']}} ...
);
...
for i = all_trial_indices
    ...
    ipsi_fields_main    = all_fields(contains(all_fields, ipsi_label)   & ~contains(all_fields, 'catch'));
    contra_fields_main  = all_fields(contains(all_fields, contra_label) & ~contains(all_fields, 'catch'));
...
for i = trial_indices
    ...
    if strcmpi(raw_delay, 'Random')
        has_delay_subfields = isfield(cond_data, [ipsi_label '_0']) || ...
                              isfield(cond_data, [ipsi_label '_100']) || ...
                              isfield(cond_data, [ipsi_label '_200']);

        if has_delay_subfields
            delays = [0, 100, 200];
            for delay_val = delays
                for polarity = {ipsi_label, contra_label}
                    side_label = sprintf('%s_%d', polarity{1}, delay_val);
                    if ~isfield(cond_data, side_label), continue; end
                    try
                        base_data = cond_data;
    ...
    % Loop over sides
    for side = {ipsi_label, contra_label}
        side_label = side{1};
        try
            fig = plot_traces_neural(base_data, cond_data, side_label, false, ...
                meta_cond, true, show_figs);
