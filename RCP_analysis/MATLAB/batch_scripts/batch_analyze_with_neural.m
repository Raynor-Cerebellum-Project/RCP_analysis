...
% Pre-initialize per-field fr_traces sum and count trackers
...
for f = 2:length(baseline_file_nums)
    ...
    for field = fields'
        key = field{1};
        if ~isfield(current, key) || ~isfield(merged_baseline, key)
            continue;
        end

        % Ensure field exists in sum structs
        if ~isfield(fr_sum_struct, key)
            fr_sum_struct.(key) = cell(size(merged_baseline.(key).fr_traces));
            fr_count_struct.(key) = zeros(size(merged_baseline.(key).fr_traces));
            for ch = 1:length(fr_sum_struct.(key))
                fr_sum_struct.(key){ch} = merged_baseline.(key).fr_traces{ch};
                fr_count_struct.(key)(ch) = size(merged_baseline.(key).fr_traces{ch}, 1);
            end
        end
        ...
        for subf = fnames'
            ...
            if strcmp(subkey, 'fr_traces')
                % Sum fr_traces per channel
                for ch = 1:length(current.(key).fr_traces)
                    new_data = current.(key).fr_traces{ch};
                    if ~isempty(new_data)
                        fr_sum_struct.(key){ch} = [
                            fr_sum_struct.(key){ch};
                            new_data
                        ];
                        fr_count_struct.(key)(ch) = fr_count_struct.(key)(ch) + size(new_data, 1);
                    end
                end
            else
                % Concatenate other subfields
...
% Overwrite the merged fr_traces per field
for key = fieldnames(fr_sum_struct)'
    fieldname = key{1};
