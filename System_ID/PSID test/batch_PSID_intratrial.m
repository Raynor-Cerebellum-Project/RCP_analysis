...
for cond_i = 1:length(condition_files)
    ...
    % bsv
    nexttile; plot(1:n_ahead, mse_bsv, 'k-x', 'LineWidth', lw); title('bsv Rel. MSE');
    xlabel('Step Ahead (ms)'); ylabel('Relative MSE'); set(gca,'TickDir','out');

    nexttile; plot(1:n_ahead, r_bsv, 'b-x', 'LineWidth', lw); title('bsv Corr');
    xlabel('Step Ahead (ms)'); ylabel('Correlation'); set(gca,'TickDir','out');

    nexttile;
    s = 10; t = (1:size(Zpb,1)) + s;
    plot(t/1000, Ztb(:,s), 'b', 'LineWidth', lw); hold on;
    plot(t/1000, Zpb(:,s), 'r--', 'LineWidth', lw);
    title('bsv @10ms'); xlabel('Time (s)'); ylabel('Body velocity (deg/s)');

    nexttile;
    s = 30; t = (1:size(Zpb,1)) + s;
    plot(t/1000, Ztb(:,s), 'b', 'LineWidth', lw); hold on;
    plot(t/1000, Zpb(:,s), 'r--', 'LineWidth', lw);
    title('bsv @30ms'); xlabel('Time (s)'); ylabel('Body velocity (deg/s)');

    % hsv
    nexttile; plot(1:n_ahead, mse_hsv, 'k-x', 'LineWidth', lw); title('hsv Rel. MSE');
    xlabel('Step Ahead (ms)'); ylabel('Relative MSE'); set(gca,'TickDir','out');

    nexttile; plot(1:n_ahead, r_hsv, 'b-x', 'LineWidth', lw); title('hsv Corr');
    xlabel('Step Ahead (ms)'); ylabel('Correlation'); set(gca,'TickDir','out');

    nexttile;
    s = 10; t = (1:size(Zph,1)) + s;
    plot(t/1000, Zth(:,s), 'b', 'LineWidth', lw); hold on;
    plot(t/1000, Zph(:,s), 'r--', 'LineWidth', lw);
    title('hsv @10ms'); xlabel('Time (s)'); ylabel('Head velocity (deg/s)');

    nexttile;
    s = 30; t = (1:size(Zph,1)) + s;
    plot(t/1000, Zth(:,s), 'b', 'LineWidth', lw); hold on;
    plot(t/1000, Zph(:,s), 'r--', 'LineWidth', lw);
    title('hsv @30ms'); xlabel('Time (s)'); ylabel('Head velocity (deg/s)');

    % Activate an unused tile just for legend
    % Add legend directly inside tile 8
    nexttile(8); % Reuse tile 8 where hsv @30ms is plotted
    hold on;
    h_true = plot(nan, nan, 'b-', 'LineWidth', lw);
    h_pred = plot(nan, nan, 'r--', 'LineWidth', lw);

    legend([h_true, h_pred], {'True', 'Predicted'}, ...
    ...
    set(findall(fig, '-property', 'FontSize'), 'FontSize', font_size);
        % Save figure in both PDF and SVG to separate folders
